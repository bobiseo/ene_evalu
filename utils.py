import re
import pandas as pd
from pathlib import Path


_BRACKET_PAT = re.compile(r"""
    ^\s*              # optional leading spaces
    \[                # opening bracket
    \s*['"]?          # optional quote after [
    (?P<label>[^'"]+) # the label itself
    ['"]?\s*          # optional quote before ]
    \]\s*$            # closing bracket + optional spaces
""", re.VERBOSE)

def _unwrap(x):
    """
    Return a clean scalar label.
    • ['Suggestion']                 -> 'Suggestion'
    • "['Suggestion']" (str)         -> 'Suggestion'
    • ('Payment',)  (singleton tuple)-> 'Payment'
    • Anything else                  -> x  (unchanged)
    """
    if isinstance(x, (list, tuple)) and len(x) == 1:
        return _unwrap(x[0])   
    if isinstance(x, str):
        m = _BRACKET_PAT.match(x)
        if m:
            return m.group("label")              # strip the brackets/quotes
        return x                                 # already clean
    return x

def chain_score(row) -> float:
    """
    0.00  if intent wrong
    0.33  if only intent right
    0.66  if intent + tone right
    1.00  if all three right
    """
    if row["intent_pred"] != row["intent_true"]:
        return 0.0
    score = 1.0 / 3
    if row["tone_pred"] == row["tone_true"]:
        score += 1.0 / 3
        if row["resolution_pred"] == row["resolution_true"]:
            score += 1.0 / 3
    return round(score, 2)

def save_chained_results(preds, data, df, out_dir: str, model_key: str):
    out_path = Path(out_dir)
    out_path.mkdir(parents=True, exist_ok=True)
    csv_path = out_path / f"predictions_{model_key}.csv"

    y_test_df = data.y_test_df().reset_index(drop=True)
    texts     = df.reset_index(drop=True).iloc[data.X_test_idx]["text"].tolist()
   
    # Build the output, using _unwrap to strip any singleton lists:
    result_df = pd.DataFrame({
        "text"           : texts,
        "intent_true"    : [ _unwrap(s) for s in y_test_df["intent"] ],
        "tone_true"      : [ _unwrap(s).split(" | ")[1] for s in y_test_df["combo_23"] ],
        "resolution_true": [ _unwrap(s).split(" | ")[2] for s in y_test_df["combo_234"] ],
        "intent_pred"    : [ _unwrap(s) for s in preds["intent"] ],
        "tone_pred"      : [ _unwrap(s).split(" | ")[1] for s in preds["combo_23"] ],
        "resolution_pred": [ _unwrap(s).split(" | ")[2] for s in preds["combo_234"] ],
    })

    result_df["score"] = result_df.apply(chain_score, axis=1)
    result_df.to_csv(csv_path, index=False)
    print(f"📄 {csv_path} saved")
    return result_df
